...
module.exports.forgetPassword = async (email) =>{
  ...
  // create reset token expiry time = genrate link data and time +_ extra 15 min
  // token will be validation only 15 minutes, after 15 minutes token will be expiry and you can't reset your password 
  await user.save();

  const resetLink = `http://localhost:3500/reset-password/${token}`
  // resetLink = fronted page link that shoe newPassword filed with filed with change password btn
